...
| `surface` | `panel` na pasku bocznym, `dialog` w oknie dołączania, `page` na pełnym ekranie, `status` w panelu Work Status |
...
| `openCommit(sha)` | otworzyć widok Diff na tym commicie bieżącego projektu. `sha` ma od 7 do 64 znaków szesnastkowych; OpenChamber sam odczytuje commit. |
| `setHeight(px)` | ustawić wysokość twojej sekcji Work Status. OpenChamber trzyma ją między 24 a 320 pikselami; wyższa treść przewija się wewnątrz strony. W innych miejscach jest ignorowane. |
...
| `NO_DIRECTORY` | względna ścieżka pliku użyta bez otwartego projektu albo `openCommit` bez otwartego projektu |
| `NOT_FOUND` | `readFile` na pliku, który nie istnieje, albo `openCommit` na commicie, którego nie ma w projekcie |
...
| `UNSUPPORTED` | ten OpenChamber nie potrafi wykonać wywołania, na przykład `openCommit` tam, gdzie nie ma widoku Diff |
