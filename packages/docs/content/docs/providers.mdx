...
3. Optionally list a model's reasoning levels (for example `low, medium, high`). Each level shows up in the model's variant picker in chat, the same as for built-in providers.
4. Optionally add request headers.
5. Save — OpenChamber writes the provider block to OpenCode config and stores the key in OpenCode auth (literal keys) or records an `{env:VAR}` reference.
6. To change an existing custom provider, open it and choose **Edit**.
