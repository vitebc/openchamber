...
| `surface` | `panel` на рейці, `dialog` у вікні прикріплення, `page` на повному екрані, `status` у панелі Work Status |
...
| `openCommit(sha)` | відкрити Diff view на цьому коміті поточного проєкту. `sha` має від 7 до 64 шістнадцяткових символів; OpenChamber сам читає коміт. |
| `setHeight(px)` | задати висоту вашої секції Work Status. OpenChamber тримає її в межах від 24 до 320 пікселів; вищий вміст прокручується всередині вашої сторінки. В інших місцях ігнорується. |
...
| `NO_DIRECTORY` | відносний шлях до файлу використано без відкритого проєкту, або `openCommit` без відкритого проєкту |
| `NOT_FOUND` | `readFile` файлу, якого не існує, або `openCommit` на коміті, якого немає в проєкті |
...
| `UNSUPPORTED` | цей OpenChamber не вміє виконати виклик, наприклад `openCommit` там, де немає Diff view |
