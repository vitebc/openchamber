...
Your page can tell where it is from `ctx.surface`: `panel`, `dialog`, `page`, or `status`. When the user clicks an attached item, its `data` comes back in `ctx.item`. Opening from the rail, + menu, or full-screen page menu starts without an item.
...
## A section in Work Status

`contributes.statusSection` adds your own section to the Work Status panel next to the chat. Users turn it on and off and move it in that panel's section list, like the built-in sections.

```json
"statusSection": { "entry": "status/index.html", "title": "Recent commits", "height": 160 }
```

- `true` reuses `panel.entry`.
- `entry` is a separate HTML page in your folder. With it you don't need `panel.entry`, so the extension can be just this section, with no rail icon.
- `title` replaces the panel name in the section header, up to 60 characters.
- `height` is the starting height in pixels, from 24 to 320 (120 by default). Call [`host.setHeight()`](/sdk/host/) when your content changes size.

Your page sees `ctx.surface` as `status`. OpenChamber loads it only while the Work Status panel is visible and your section is expanded, and unloads it when the user collapses the section, so keep nothing there you can't rebuild. A section-only extension can use capabilities, a service, an integration, and files, but not `page`, `attach`, `actions`, or `commands`. Web and desktop only. The `git-graph-status` example is a complete section.
...
An extension that only styles tools needs no page at all: leave out `panel.entry` and it has no rail icon, just a card in Settings → Extensions. Without a page it may declare `tools` and nothing else. A Work Status section with its own `entry` is the other exception, see above.
