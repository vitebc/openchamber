...
Build it from the folder with `bunx openchamber-guest-bundle panel/main.ts panel/main.js`, then add the folder at Settings → Extensions. Six more examples (every UI control, a task list with actions and a slash command, a local service, a config file editor, a tools-only package, a Work Status section with a git graph) are at [github.com/openchamber/openchamber/tree/main/packages/sdk/examples](https://github.com/openchamber/openchamber/tree/main/packages/sdk/examples).
