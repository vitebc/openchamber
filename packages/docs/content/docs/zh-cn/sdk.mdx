...
`ctx.surface` 为 `panel`、`dialog`、`page` 或 `status`。点击附加条目时，`ctx.item` 包含其数据；其他情况下为 `null`。
...
`contributes.statusSection` 会在聊天旁的 Work Status 面板里添加你自己的分区。用户可以在该面板的分区列表中开关和移动它，就像内置分区一样。
...
- `true` 复用 `panel.entry`。
- `entry` 是你文件夹里单独的 HTML 页面。有了它就不需要 `panel.entry`，扩展可以只有这个分区，没有侧栏图标。
- `title` 替换分区标题中的面板名称，最多 60 个字符。
- `height` 是初始高度（像素），24 到 320，默认 120。内容尺寸变化时调用 [`host.setHeight()`](/sdk/host/)。

你的页面看到的 `ctx.surface` 是 `status`。只有在 Work Status 面板可见且你的分区展开时，OpenChamber 才会加载它；用户折叠分区时会卸载，所以不要在里面保存无法重建的状态。只有分区的扩展可以使用权限、服务、集成和文件，但不能用 `page`、`attach`、`actions` 或 `commands`。仅限 Web 和桌面版。示例 `git-graph-status` 就是一个完整的分区。
...
只给工具定制样式的扩展根本不需要页面：不写 `panel.entry`，它就没有侧栏图标，只在 设置 → 扩展 里有一张卡片。没有页面时，它可以只声明 `tools`，别的什么都不写。另一个例外是带自己 `entry` 的 Work Status 分区，见上文。
