...
Compile a partir da pasta com `bunx openchamber-guest-bundle panel/main.ts panel/main.js` e depois adicione a pasta em Configurações → Extensões. Mais seis exemplos (todos os controles de UI, uma lista de tarefas com ações e um comando de barra, um serviço local, um editor de arquivo de configuração, um pacote somente com ferramentas, uma seção do Work Status com um grafo do git) estão em [github.com/openchamber/openchamber/tree/main/packages/sdk/examples](https://github.com/openchamber/openchamber/tree/main/packages/sdk/examples).
