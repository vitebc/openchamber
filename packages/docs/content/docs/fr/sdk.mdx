...
`ctx.surface` vaut `panel`, `dialog`, `page` ou `status`. Un clic sur un élément attaché place ses données dans `ctx.item`, sinon cette valeur est `null`.
...
## Une section dans Work Status

`contributes.statusSection` ajoute votre propre section au panneau Work Status à côté du chat. Les utilisateurs l'activent, la désactivent et la déplacent dans la liste des sections de ce panneau, comme les sections intégrées.

```json
"statusSection": { "entry": "status/index.html", "title": "Recent commits", "height": 160 }
```

- `true` réutilise `panel.entry`.
- `entry` est une page HTML séparée dans votre dossier. Avec elle, `panel.entry` n'est pas nécessaire, et l'extension peut n'être que cette section, sans icône dans la barre latérale.
- `title` remplace le nom du panneau dans l'en-tête de la section, jusqu'à 60 caractères.
- `height` est la hauteur de départ en pixels, de 24 à 320 (120 par défaut). Appelez [`host.setHeight()`](/sdk/host/) quand la taille de votre contenu change.

Votre page voit `ctx.surface` à `status`. OpenChamber ne la charge que tant que le panneau Work Status est visible et votre section dépliée, et la décharge quand l'utilisateur replie la section. N'y gardez donc rien que vous ne puissiez reconstruire. Une extension réduite à une section peut utiliser des permissions, un service, une intégration et des fichiers, mais pas `page`, `attach`, `actions` ni `commands`. Web et bureau uniquement. L'exemple `git-graph-status` est une section complète.
...
Une extension qui ne fait que styliser des outils n'a besoin d'aucune page : omettez `panel.entry` et elle n'aura pas d'icône dans la barre latérale, juste une carte dans Paramètres → Extensions. Sans page, elle peut déclarer `tools` et rien d'autre. L'autre exception est une section Work Status avec son propre `entry`, voir plus haut.
