...
| `surface` | レールは `panel`、添付ウィンドウは `dialog`、全画面は `page`、Work Status パネルは `status` |
...
| `openCommit(sha)` | 現在のプロジェクトのそのコミットで Diff ビューを開く。`sha` は 7〜64 文字の 16 進数で、コミットは OpenChamber 自身が読み込みます。 |
| `setHeight(px)` | Work Status セクションの高さを設定する。OpenChamber は 24〜320 ピクセルに収め、それより高い内容はページ内でスクロールします。他の場所では無視されます。 |
...
| `NO_DIRECTORY` | 開いているプロジェクトがない状態で相対ファイルパスを使った、または開いているプロジェクトがない状態で `openCommit` を呼んだ |
| `NOT_FOUND` | 存在しないファイルに対する `readFile`、またはプロジェクトにないコミットへの `openCommit` |
...
| `UNSUPPORTED` | この OpenChamber では実行できない呼び出し。例: Diff ビューがない場所での `openCommit` |
