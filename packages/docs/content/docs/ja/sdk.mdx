...
`ctx.surface` は `panel`、`dialog`、`page`、`status` のいずれかです。添付項目をクリックすると、そのデータが `ctx.item` に入り、それ以外は `null` です。
...
`contributes.statusSection` は、チャットの横にある Work Status パネルに独自のセクションを追加します。ユーザーは組み込みのセクションと同じように、そのパネルのセクション一覧でオン・オフや並べ替えができます。

```json
"statusSection": { "entry": "status/index.html", "title": "Recent commits", "height": 160 }
```

- `true` は `panel.entry` をそのまま使います。
- `entry` はフォルダ内の別の HTML ページです。これがあれば `panel.entry` は不要なので、レールのアイコンなしでこのセクションだけの拡張機能にできます。
- `title` はセクション見出しのパネル名を置き換えます。最大 60 文字です。
- `height` はピクセル単位の初期の高さで、24〜320 (既定は 120) です。内容のサイズが変わったら [`host.setHeight()`](/sdk/host/) を呼んでください。

ページから見た `ctx.surface` は `status` です。OpenChamber は Work Status パネルが表示され、セクションが展開されている間だけページを読み込み、ユーザーがセクションを折りたたむと破棄します。作り直せない状態はそこに置かないでください。セクションだけの拡張機能は権限、サービス、連携、ファイルを使えますが、`page`、`attach`、`actions`、`commands` は使えません。Web とデスクトップのみです。`git-graph-status` の例が完成したセクションです。
...
ツールの見た目を整えるだけの拡張機能にはページはまったく不要です。`panel.entry` を省略すると、レールにアイコンは出ず、設定 → 拡張機能 にカードが表示されるだけになります。ページがない場合は `tools` だけを宣言して他は何も宣言しなくて構いません。もう一つの例外は、独自の `entry` を持つ Work Status セクションです(上記参照)。
