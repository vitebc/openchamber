...
`ctx.surface` ist `panel`, `dialog`, `page` oder `status`. Beim Klick auf einen angehängten Eintrag enthält `ctx.item` dessen Daten, sonst `null`.
...
## Ein Abschnitt in Work Status

`contributes.statusSection` fügt dem Work-Status-Panel neben dem Chat einen eigenen Abschnitt hinzu. Nutzer schalten ihn in der Abschnittsliste dieses Panels ein und aus und verschieben ihn dort, wie die eingebauten Abschnitte.

```json
"statusSection": { "entry": "status/index.html", "title": "Recent commits", "height": 160 }
```

- `true` verwendet `panel.entry`.
- `entry` ist eine eigene HTML-Seite in deinem Ordner. Damit brauchst du kein `panel.entry`, die Erweiterung kann also nur aus diesem Abschnitt bestehen, ohne Icon in der Leiste.
- `title` ersetzt den Panel-Namen in der Abschnittsüberschrift, bis zu 60 Zeichen.
- `height` ist die Anfangshöhe in Pixeln, 24 bis 320 (Standard 120). Rufe [`host.setHeight()`](/sdk/host/) auf, wenn sich die Größe deines Inhalts ändert.

Deine Seite sieht `ctx.surface` als `status`. OpenChamber lädt sie nur, solange das Work-Status-Panel sichtbar und dein Abschnitt aufgeklappt ist, und entlädt sie, wenn der Nutzer ihn zuklappt. Halte dort also nichts, was du nicht neu aufbauen kannst. Eine Erweiterung nur mit Abschnitt darf Berechtigungen, einen Dienst, eine Integration und Dateien nutzen, aber nicht `page`, `attach`, `actions` oder `commands`. Nur Web und Desktop. Das Beispiel `git-graph-status` ist ein vollständiger Abschnitt.
...
Eine Erweiterung, die nur Tools gestaltet, braucht gar keine Seite: lass `panel.entry` weg, dann hat sie kein Icon in der Leiste, nur eine Karte unter Einstellungen → Erweiterungen. Ohne Seite darf sie `tools` und sonst nichts deklarieren. Die andere Ausnahme ist ein Work-Status-Abschnitt mit eigenem `entry`, siehe oben.
