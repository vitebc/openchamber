...
폴더에서 `bunx openchamber-guest-bundle panel/main.ts panel/main.js`로 빌드한 다음, 설정 → 확장 에서 그 폴더를 추가하세요. 예제 여섯 개(모든 UI 컨트롤, 작업과 슬래시 명령이 있는 작업 목록, 로컬 서비스, 설정 파일 편집기, 도구 전용 패키지, git 그래프가 있는 Work Status 섹션)가 [github.com/openchamber/openchamber/tree/main/packages/sdk/examples](https://github.com/openchamber/openchamber/tree/main/packages/sdk/examples)에 더 있습니다.
